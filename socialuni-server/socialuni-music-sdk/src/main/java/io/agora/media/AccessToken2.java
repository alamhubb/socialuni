...
public class AccessToken2 {
    ...
    public AccessToken2(String appId, String appCert, int expire) {
        ...
        this.appId = appId;
        this.expire = expire;
        this.issueTs = io.agora.media.Utils.getTimestamp();
        this.salt = io.agora.media.Utils.randomInt();
    }
    ...
    public String build() throws Exception {
        if (!io.agora.media.Utils.isUUID(this.appId) || !io.agora.media.Utils.isUUID(this.appCert)) {
            return "";
        }

        io.agora.media.ByteBuf buf = new io.agora.media.ByteBuf().put(this.appId).put(this.issueTs).put(this.expire).put(this.salt)
        ...
        io.agora.media.ByteBuf bufferContent = new io.agora.media.ByteBuf();
        bufferContent.put(signature);
        bufferContent.buffer.put(buf.asBytes());

        return getVersion() + io.agora.media.Utils.base64Encode(io.agora.media.Utils.compress(bufferContent.asBytes()));
    }
    ...
    public byte[] getSign() throws Exception {
        Mac mac = Mac.getInstance("HmacSHA256");
        mac.init(new SecretKeySpec(new io.agora.media.ByteBuf().put(this.issueTs).asBytes(), "HmacSHA256"));
        byte[] signing = mac.doFinal(this.appCert.getBytes());
        mac.init(new SecretKeySpec(new io.agora.media.ByteBuf().put(this.salt).asBytes(), "HmacSHA256"));
    ...
    public boolean parse(String token) {
        if (!getVersion().equals(token.substring(0, io.agora.media.Utils.VERSION_LENGTH))) {
            return false;
        }

        try {
            byte[] data = io.agora.media.Utils.decompress(io.agora.media.Utils.base64Decode(token.substring(Utils.VERSION_LENGTH)));
            io.agora.media.ByteBuf buff = new io.agora.media.ByteBuf(data);
    ...
    public static class Service {
        ...
        public io.agora.media.ByteBuf pack(io.agora.media.ByteBuf buf) {
            return buf.put(this.type).putIntMap(this.privileges);
        }

        public void unpack(io.agora.media.ByteBuf byteBuf) {
