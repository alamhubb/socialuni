...
import io.agora.media.PackableEx;
...
public class AccessToken {
    ...
    public String build() throws Exception {
        if (! io.agora.media.Utils.isUUID(appId)) {
            return "";
        }

        if (!io.agora.media.Utils.isUUID(appCertificate)) {
            return "";
        }

        messageRawContent = io.agora.media.Utils.pack(message);
        ...
        byte[] content = io.agora.media.Utils.pack(packContent);
        return getVersion() + this.appId + io.agora.media.Utils.base64Encode(content);
    }
    ...
    public static byte[] generateSignature(String appCertificate, 
        ...
        return io.agora.media.Utils.hmacSign(appCertificate, baos.toByteArray());
    }
    ...
    public boolean fromString(String token) {
        ...
        try {
            appId = token.substring(io.agora.media.Utils.VERSION_LENGTH, io.agora.media.Utils.VERSION_LENGTH + io.agora.media.Utils.APP_ID_LENGTH);
            PackContent packContent = new PackContent();
            io.agora.media.Utils.unpack(io.agora.media.Utils.base64Decode(token.substring(io.agora.media.Utils.VERSION_LENGTH + io.agora.media.Utils.APP_ID_LENGTH, token.length())), packContent);
            ...
            io.agora.media.Utils.unpack(messageRawContent, message);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
    ...
    public class PrivilegeMessage implements PackableEx {
        ...
        public PrivilegeMessage() {
            salt = io.agora.media.Utils.randomInt();
            ts = Utils.getTimestamp() + 24 * 3600;
            messages = new TreeMap<>();
        }

        @Override
        public io.agora.media.ByteBuf marshal(io.agora.media.ByteBuf out) {
            return out.put(salt).put(ts).putIntMap(messages);
        }

        @Override
        public void unmarshal(io.agora.media.ByteBuf in) {
            salt = in.readInt();
            ts = in.readInt();
