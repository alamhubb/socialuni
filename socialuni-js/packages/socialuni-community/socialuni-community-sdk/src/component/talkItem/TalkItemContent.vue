    ...
    <view>
        ...
        <view v-if="!talk.globalTop || talk.globalTop===1" class="row-col-center px-sm row-wrap"
            ...
            <view class="q-tag mt-xss">
                ...
                <!--        有市区的名称就不显示省的名称-->
                <text v-if="!talk.district.cityName || !talk.district.districtName">{{
...
export default class TalkItemContent extends Vue {
    ...
    previewImage(index) {
        SocialuniAppUtil.UniUtil.previewImage(index, this.talk.imgs.map(item => this.getTalkLargeImgUrl(this.talk.user.id, item.src)))
    }
