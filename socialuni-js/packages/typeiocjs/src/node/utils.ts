...
import { CLIENT_ENTRY, CLIENT_PUBLIC_PATH, ENV_PUBLIC_PATH, FS_PREFIX, OPTIMIZABLE_ENTRY_RE, loopbackHosts, wildcardHosts, } from './constants';
import type { DepOptimizationConfig } from './optimizer';
import type { ResolvedConfig } from './config';
import type { ResolvedServerUrls, ViteDevServer } from './server';
import type { PreviewServer } from './preview';
import { type PackageCache, findNearestPackageData, resolvePackageData, } from './packages';
...
const limitFlattenIdLength = (id: string, limit: number = FLATTEN_ID_MAX_FILE_LENGTH): string => {
    if (id.length <= limit) {
        return id;
    }
...
export function moduleListContains(moduleList: string[] | undefined, id: string): boolean | undefined {
...
export function resolveDependencyVersion(dep: string, pkgRelativePath = '../../package.json'): string {
...
export function createDebugger(namespace: ViteDebugScope, options: DebuggerOptions = {}): debug.Debugger['log'] | undefined {
...
export async function asyncReplace(input: string, re: RegExp, replacer: (match: RegExpExecArray) => string | Promise<string>): Promise<string> {
...
export function lookupFile(dir: string, fileNames: string[]): string | undefined {
    while (dir) {
...
export function generateCodeFrame(source: string, start: number | Pos = 0, end?: number | Pos): string {
...
export function processSrcSet(srcs: string, replacer: (arg: ImageCandidate) => Promise<string>): Promise<string> {
...
export function combineSourcemaps(filename: string, sourcemapList: Array<DecodedSourceMap | RawSourceMap>): RawSourceMap {
...
export function diffDnsOrderChange(oldUrls: ViteDevServer['resolvedUrls'], newUrls: ViteDevServer['resolvedUrls']): boolean {
...
export async function resolveServerUrls(server: Server, options: CommonServerOptions, config: ResolvedConfig): Promise<ResolvedServerUrls> {
...
export function mergeConfig<D extends Record<string, any>, O extends Record<string, any>>(defaults: D extends Function ? never : D, overrides: O extends Function ? never : O, isRoot = true): Record<string, any> {
...
export const teardownSIGTERMListener = (callback: () => Promise<void>): void => {
