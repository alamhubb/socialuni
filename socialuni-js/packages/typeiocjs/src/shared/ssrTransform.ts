 ...
 */
export function analyzeImportedModDifference(mod: any, rawId: string, moduleType: string | undefined, metadata?: SSRImportBaseMetadata): void {
    ...
    if (metadata?.importedNames?.length) {
        ...
        if (missingBindings.length) {
            ...
            if (moduleType === 'module') {
                throw new SyntaxError(`[vite] The requested module '${rawId}' does not provide an export named '${lastBinding}'`);
            }
            else {
