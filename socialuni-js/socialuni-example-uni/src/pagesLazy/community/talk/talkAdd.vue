        ...
        />
            ...
            <view class="row-col-center pd-sm">
                <div class="flex-row text-md">
                    可见范围：
                    <view class="text-gray row-all-center bg-grey10 w100 mr-xs px-xs"
                          @click="showVisibleTypeSelect=true">
                    ...
                    <template v-if="GenderTypeAll === appGenderType">
                        <div>，</div>
                        <view class="text-gray row-all-center bg-grey10 w100 mx-xs px-xs"
                              @click="showVisibleGenderSelect=true">
...
export default class TalkAddView extends Vue {
    ...
    async chooseImage() {
        ...
        if (this.cosAuthRO) {
            ...
            SocialuniAppAPI.sendErrorLogAPI(null, SocialuniAppUtil.RouterUtil.getCurrentPageURI(), '用户发表动态失败，未获取上传图片所需要的认证信息')
