  ...
  <div class="h100p">
    <div class="flex-row">
      ...
      <q-label-item label="用户名">
        <el-input v-model="operateUser.nickname"></el-input>
      </q-label-item>
      ...
      <label-item label="位置和经纬度：">
        <el-button class="mr-sm" size="small" type="primary" @click="openMapDialog(operateUser)">选择位置
        </el-button>
        <div>
          {{ operateUser.district }}：{{ operateUser.lat }}，{{ operateUser.lng }}
        </div>
      </label-item>
    </div>
...
class OperateUserManage extends Vue {
  ...
  peiwanList: SocialuniUserImgComRO[] = []
  ...
  operateUser: PeiwanRO = new PeiwanRO()
  ...
  async addPeiwanAPI() {
    await SocialuniPeiwanAdminAPI.addPeiwanInfoAPI(this.operateUser)
    this.operateUser = new PeiwanRO()
    this.queryPeiwanListAPI()
  }

  async addOperateUserAPI() {
    await SocialuniOperateUserAPI.addOperateUser(this.operateUser)
    this.operateUser = new PeiwanRO()
    this.queryPeiwanListAPI()
  }
  ...
  async queryPeiwanListAPI() {
    ...
    const res = await SocialuniOperateUserAPI.queryOperateUserList()
  ...
  async avatarImgChange(file: any) {
    ...
    this.operateUser.avatar = imgFile.src
  ...
  commonSava() {
    if (this.operateUser.nickname) {
      this.addOperateUserAPI()
    } else {
