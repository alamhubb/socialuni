package com.socialuni.center.web.service.user;/*
...
import com.socialuni.center.web.utils.GenderUtil;
import com.socialuni.center.web.utils.GenerateNicknameUtil;
...
//            user.setLastOnlineTime(new Date());
    ...
    public UniUserDO createUser(SocialProviderLoginQO loginData) {
        String nickname = loginData.getNickName();
        String provider = loginData.getProvider();
        String platform = loginData.getPlatform();
        String avatar = loginData.getAvatarUrl();
        String city = loginData.getCity();

        //处理性别
        GenderTypeNumEnum genderTypeNumEnum = GenderTypeNumEnum.getEnumByValue(loginData.getGender());
        String gender;
        if (genderTypeNumEnum != null) {
            gender = genderTypeNumEnum.getName();
        } else {
            gender = GenderTypeNumEnum.female.getName();
        }
        ...
        if (provider.equals(ProviderType.qq) && platform.equals(PlatformType.app)) {
            String birthday = null;
            if (StringUtils.isNotEmpty(loginData.getBirthday())) {
                birthday = DateUtils.getYearBirthDateByDateStr(loginData.getBirthday());
            }
...
import com.socialuni.sdk.utils.CommonUserROUtil;
...
public class UserService {
    ...
    public SocialUserRO getMineUser() {
        ...
        SocialUserRO userRO = CommonUserROUtil.getMineUser(user);
        return userRO;
    }
}
*/
