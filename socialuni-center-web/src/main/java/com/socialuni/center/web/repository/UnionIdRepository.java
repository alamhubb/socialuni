...
public interface UnionIdRepository extends JpaRepository<UnionIdDO, Long> {
    @Caching(
            put = {
                    ...
                    @CachePut(cacheNames = "getUnionIdByCom", key = "#unionIdDO.contentType+'-'+#unionIdDO.contentId+'-'+#unionIdDO.status+'-'+#unionIdDO.devId+'-'+#unionIdDO.userId")
            }
    )
    ...
    @Cacheable(cacheNames = "getUnionIdByCom", key = "#contentType+'-'+#contentId+'-'+#status+'-'+#devId+'-'+#userId")
    Optional<UnionIdDO> findFirstByContentTypeAndContentIdAndStatusAndDevIdAndUserIdOrderByIdDesc(String contentType, Integer contentId, String status, Integer devId, Integer userId);
