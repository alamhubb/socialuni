...
export default class SButton extends Vue {
  ...
  get buttonClass () {
    let buttonClass = `${this.btnDisabled ? 'use-disabled' : 'use-click'} `
    if (this.text) {
      buttonClass += `bd-none bg-theme-light color-${this.curTheme}`
    } else {
      buttonClass += `q-chunk${this.curTheme ? ('-' + this.curTheme) : ''}${this.light ? ('_light') : ''}${this.plain ? ('_plain') : ''} q-box-md`
      if (this.round) {
