...
public interface CommentRepository extends JpaRepository<SocialuniCommentDO, Integer> {
    ...
    SocialuniCommentDO findOneBySocialuniUid(String id);
    ...
    SocialuniCommentDO save(SocialuniCommentDO comment);

    //    展示评论
    @Cacheable(cacheNames = "talkComments5", key = "{#talkId}")
    List<SocialuniCommentDO> findTop5ByTalkIdAndStatusInAndParentCommentIdIsNullOrderByUpdateTimeDesc(Integer talkId, List<String> status);


    List<SocialuniCommentDO> findTop10ByUserIdOrderByUpdateTimeDesc(Integer userId);

    //    talk详情页展示评论
    @Cacheable(cacheNames = "talkComments50", key = "{#talkId}")
    List<SocialuniCommentDO> findTop50ByTalkIdAndStatusInAndParentCommentIdIsNullOrderByUpdateTimeDesc(Integer talkId, List<String> status);

    @Cacheable(cacheNames = "commentComments3", key = "{#commentId}")
    List<SocialuniCommentDO> findTop3ByParentCommentIdAndStatusInOrderByUpdateTimeDesc(Integer commentId, List<String> status);

    //   展示子回复,避免性能问题，限制最多50条，再多不如私聊了
    @Cacheable(cacheNames = "commentComments50", key = "{#commentId}")
    List<SocialuniCommentDO> findTop50ByParentCommentIdAndStatusInOrderByUpdateTimeDesc(Integer commentId, List<String> status);
    ...
    Optional<SocialuniCommentDO> findOneByUnionIdAndStatusIn(Integer id, List<String> status);

    //获取最新的评论
    SocialuniCommentDO findFirstByTalkIdOrderByIdDesc(Integer talkId);


    List<SocialuniCommentDO> findByParentCommentId(Integer commentId);


    //查询关键词触发次数时使用
    Page<SocialuniCommentDO> findByStatusNotInOrderByIdDesc(Pageable pageable, List<String> status);
}