...
*/
    ...
    //根据typeid获取所有
    public List<SocialCircleRO> getCirclesByTagTypeDOAndGender(TagTypeDO tagTypeDO, String appGenderType) {
        List<SocialCircleDO> tagDOS;
        if (appGenderType.equals(GenderType.all)) {
            return getCirclesByTagTypeId(tagTypeDO.getId());
        } else {
    ...
    //根据typeid获取所有
    public List<SocialCircleRO> getCirclesByTagTypeId(Integer tagTypeId) {
        List<SocialCircleDO> tagDOS = socialCircleRepository.findByTagTypeIdAndStatusOrderByCountDesc(tagTypeId, ContentStatus.enable);
        // 从数据库中获取tag列表
        return SocialCircleROFactory.circleDOToROS(tagDOS);
    }
}