<template>
  <data-table class="h100p" height="100" :table-data="imgAuditList" border show-page>
    ...
    <el-table-column label="是否已冻结">
      <template #default="{row}">
        {{ $const.ForbidStatus.getTypeName(row.forbidden_status) }}
      </template>
    </el-table-column>
    <!--      <el-table-column label="命中审核分类">
      <template #default="{row}">
        {{ $const.CosImgResultType.getTypeName(row.hit_flag) }}
      </template>
    </el-table-column>-->
    <el-table-column>
      <template #default="{row}">
