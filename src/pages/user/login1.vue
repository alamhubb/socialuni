...
import UserAPI from '@/plugins/social/api/center/UserAPI'
...
import UserService from '@/plugins/social/service/UserService'
import SocialUniAuthVO from '@/model/openData/SocialUniAuthVO'
import LoginAPI from '@/plugins/social/api/center/LoginAPI'
import { uniSystemModule, uniSystemStore } from '@/plugins/uniapp-sdk'
import UniSystemStoreProp from '@/plugins/uniapp-sdk/store/UniSystemStoreProp'
import ProviderUserVO from '@/model/login/ProviderUserVO'
import UniUserUtil from '@/plugins/uniapp-sdk/utils/UniUserUtil'
import OAuthAPI from '@/plugins/social/api/OAuthAPI'
...
import MockAPI from '@/plugins/social/api/MockAPI'
...
export default class LoginPage extends Vue {
  @uniSystemStore.State(UniSystemStoreProp.isWX) isWx: string
  @uniSystemStore.State(UniSystemStoreProp.isQQ) isQQ: string
  ...
  async loginByPhoneNumAndBindPhoneNum () {
    ...
    if (this.openTypeBtnEnable) {
      ...
      if (this.user) {
        ...
        // loginData.provider = UniProviderType.phone
        loginData.platform = uniSystemModule.platform
        // await SocialLoginService.providerLogin(UniProviderType.phone, loginData)
      }
      this.goBackPage()
  ...
  async providerLoginClick () {
    ...
    // userModule.loginAfterSetUserAndToken(data)
  }

  async mockSocialLogin () {
    //一行代码就可以获取登录所需要的信息, 还可以配合后台使用，一键登录，记住用户
    const { data } = await MockAPI.mockOAuthUserInfoAPI()
    const loginQO: UniProviderLoginQO = new UniProviderLoginQO()
    //如果为小程序的话不传值，默认为小程序类型
    loginQO.provider = 'social'

    loginQO.code = data.token

    const userInfoRO: SocialUserRO = data.user
    loginQO.openId = userInfoRO.id || ''
    loginQO.unionId = userInfoRO.id || ''

    loginQO.nickName = userInfoRO.nickname || ''
    loginQO.avatarUrl = userInfoRO.avatar || ''
    loginQO.gender = userInfoRO.gender || 0
    loginQO.city = userInfoRO.city || ''

    const res = await LoginAPI.providerLoginAPI(loginQO)

    userModule.loginAfterSetUserAndToken(res.data)
  }

  //mock绑定清池账号
  async mockApplyOAuth () {
    const { data } = await MockAPI.mockOAuthUserInfoAPI()
    const userInfo: UniProviderLoginQO = new UniProviderLoginQO()
    //如果为小程序的话不传值，默认为小程序类型
    userInfo.provider = 'social'

    userInfo.code = data.token

    const userInfoRO: SocialUserRO = data.user
    userInfo.openId = userInfoRO.id || ''
    userInfo.unionId = userInfoRO.id || ''

    userInfo.nickName = userInfoRO.nickname || ''
    userInfo.avatarUrl = userInfoRO.avatar || ''
    userInfo.gender = userInfoRO.gender || 0
    userInfo.city = userInfoRO.city || ''

    const res = await UserAPI.bindProviderAccountAPI(userInfo)
