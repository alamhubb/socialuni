...
import com.qingchi.web.domain.PayShellOpenChatDomain;
import com.qingchi.web.manage.ChatUserVerify;
import com.qingchi.web.service.MessageService;
...
import com.socialuni.sdk.service.chat.ChatService;
...
/**
    ...
    public ResultRO<?> readChatMessages(UserDO user, @RequestBody @Valid ChatReadVO chatVO) {
        if (user != null) {
            Integer chatId = chatVO.getChatId();
...
/*List<MessageReceiveDO> messageReceiveDOS = messageReceiveDORepository.findByChatUserIdAndIdInOrderByCreateTimeDescIdDesc(chatUserDb, readVO.getMessageIds());
    ...
    public ResultRO<List<ChatVO>> queryHomeChat(UserDO user) {
        if (user != null) {
    ...
    public ResultRO<List<ChatVO>> queryChatList(UserDO user) {
        if (user != null) {
            return new ResultRO<>(chatUserService.getChats(user));
        }
        return new ResultRO<>(chatUserService.getChats());
    ...
    public ResultRO<ChatVO> queryChat(UserDO user, @RequestBody UserIdVO receiveUserVO) {
//        Integer userId =  UnionIdDbUtil.getUserIdByUid(receiveUserVO.getUserId(),  user);


//        UserDO receiveUser = UserUtils.get(userId);
...
/*new ChatVO(chat);
    ...
    public ResultRO<ChatVO> openChat(UserDO user, @RequestBody @Valid @NotNull OpenChatVO chatVO) throws IOException {
        ...
        if (!chatDOOptional.isPresent()) {
            log.info("不存在的chat");
            throw new SocialParamsException("不存在的chat");
        }
...
/*if (!chatDO.getStatus().equals(ChatStatus.waitOpen)) {
        ...
        //查询俩人的关注状态
        boolean userFollowed = followDao.userHasFollowBeUser(receiveUserId, chatUserDO.getUserId());

        //小于1，需要付费支付
        Boolean dbNeedPayOpen = !userFollowed;

        //未关注，需要付费支付
        if (dbNeedPayOpen) {
            //需要付费支付，前台传值错误
            if (!dbNeedPayOpen.equals(needPayOpen)) {
                throw new SocialBusinessException("对方未关注您，需要付费开启会话，请刷新后重试");
            }

            */
/*Integer userShell = user.getShell();
            if (userShell < 10) {
                log.info("系统被攻击，不该触发这里，用户不够10贝壳，无法开启对话");
...
/*Optional<UserContactDO> userContactDOOptional = userContactRepository.findFirstByUserIdAndBeUserIdAndStatusAndType(user.getId(), receiveUserId, CommonStatus.normal, ExpenseType.openChat);
            ...
            //不需要付费支付，前台传值错误
            if (!dbNeedPayOpen.equals(needPayOpen)) {
                throw new SocialBusinessException("对方已经关注了您，无需支付贝壳，即可开启对话，请刷新后重试");
            }
        }
...
/*new ChatVO(chat);
    ...
    private ResultRO<ChatUserDO> checkChatUserAndStatusIsWaitOpen(Integer chatId, Integer userId) {
        ...
        if (chatUserDO.getStatus().equals(ChatUserStatus.enable)) {
            throw new SocialBusinessException("会话已开启，请刷新后重试");
        }
        return new ResultRO<>(chatUserDO);
    }

    @PostMapping("frontDeleteChat")
    public ResultRO<?> frontDeleteChat(UserDO user, @RequestBody @Valid @NotNull ChatRemoveVO chatVO) {
        Integer chatId = chatVO.getChatId();
        */
/*Optional<ChatDO> chatDOOptional = chatRepository.findFirstByIdAndStatus(chatId, ChatStatus.enable);
        if (!chatDOOptional.isPresent()) {
...
/* else if (chatUserDO.getStatus().equals(ChatUserStatus.enable) &&
    ...
    public ResultRO<?> removeChat(UserDO user, @RequestBody @Valid @NotNull ChatRemoveVO chatVO) {
        Integer chatId = chatVO.getChatId();
        */
/*Optional<ChatDO> chatDOOptional = chatRepository.findFirstByIdAndStatus(chatId, ChatStatus.enable);
        if (!chatDOOptional.isPresent()) {
...
/* else {
    ...
    public ResultRO<?> closeChat(UserDO user, @RequestBody @Valid @NotNull ChatRemoveVO chatVO) {
        Integer chatId = chatVO.getChatId();
        */
/*Optional<ChatDO> chatDOOptional = chatRepository.findFirstByIdAndStatus(chatId, ChatStatus.enable);
        if (!chatDOOptional.isPresent()) {
